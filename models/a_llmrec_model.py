...
class A_llmrec_model(nn.Module):
    def __init__(self, args):
        ...
        self.item_num = len(self.text_name_dict['title'])
    ...
    def generate(self, data):
        ...
        with torch.no_grad():
            ...
            for i in range(len(u)):
                ...
                target_item_title = self.find_item_text_single(target_item_id, title_flag=True, description_flag=False)
                # Debug: Print target item title for current sample
                print(f"DEBUG: Target item {target_item_id} title: {target_item_title}")

                interact_text, interact_ids = self.make_interact_text(seq[i][seq[i] > 0], 10)
                # Debug: Print interaction text and ids
                print(f"DEBUG: interact_text: '{interact_text}'")
                print(f"DEBUG: interact_ids: {interact_ids}")

                candidate_num = 20
                candidate_text, candidate_ids = self.make_candidate_text(seq[i][seq[i] > 0], candidate_num,
                                                                         target_item_id, target_item_title)
                # Debug: Print candidate text and ids
                print(f"DEBUG: candidate_text: '{candidate_text}'")
                print(f"DEBUG: candidate_ids: {candidate_ids}")
                ...
                # Debug: Print the final constructed prompt for current sample
                print(f"DEBUG: Final input_text for sample {i}: '{input_text}'")
        ...
        # Debug: Print entire list of text prompts
        print("DEBUG: Full text_input list:")
        for idx, txt in enumerate(text_input):
        ...
        with torch.no_grad():
            ...
            # Debug: Check tokenized input
            print("DEBUG: Tokenized input_ids shape:", llm_tokens.input_ids.shape)
            print("DEBUG: Tokenized attention_mask shape:", llm_tokens.attention_mask.shape)
            if llm_tokens.input_ids.shape[1] == 0:
                print("ERROR: Tokenized input_ids is empty!")
            ...
            with torch.cuda.amp.autocast():
                ...
                print("DEBUG: Concatenated inputs_embeds shape:", inputs_embeds.shape)
                print("DEBUG: Concatenated attention_mask shape:", attention_mask.shape)
            ...
            # Debug: Print the generated output texts
            print("DEBUG: Generated output texts:")
            for idx, out in enumerate(output_text):
                print(f"  Sample {idx}: '{out}'")

        for i in range(len(text_input)):
            with open(
                    f'/home/kavach/Dev/Publication/A-LLM-Rec/A-LLMRec_copy_original/rec_output/luxuryBeauty/debug-6git .txt',
                    'a') as f:
